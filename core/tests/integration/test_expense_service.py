...
def test_create_multiple_expenses_and_list():
    ...
    pan = service.create_expense("Pan", 3, "Mercado", date(2025, 1, 10))
    leche = service.create_expense("Leche", 4, "Supermercado", date(2025, 1, 20))
    expenses = service.list_expenses()

    assert pan in expenses
    assert leche in expenses
...
def test_remove_expense_reduces_total():
    ...
    libro = service.create_expense("Libro", 2, "Mercado", date(2025, 1, 10))
    revista = service.create_expense("Revista", 4, "Supermercado", date(2025, 1, 20))
    expenses = service.list_expenses()
    service.remove_expense(libro.id)

    assert revista in service.list_expenses()
    assert len(expenses) == 2
...
def test_update_expense_partial_fields():
    ...
    camisa = service.create_expense("Camiseta", 15, "Ropa", date(2025, 1, 10))
    service.update_expense(camisa.id, amount=18)
    camisa = service.list_expenses()[0]

    assert camisa.title == "Camiseta"
    assert camisa.amount == 18
    assert camisa.description == "Ropa"
